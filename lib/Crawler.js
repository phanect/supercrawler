 ...
 */
Crawler.prototype._crawlTick = async function () {
  var urlList,
      nextRequestDate,
      nowDate,
      self = this;
  ...
  try {
    ...
    try {
      ...
      await urlList.upsert(resultUrl);
    } finally {
      self._outstandingRequests--;
    }
  } catch(err) {
 ...
 */
Crawler.prototype._processUrl = async function (url) {
  var self = this,
      response,
      urlList;
  ...
  if (this._robotsEnabled) {
    await this._downloadAndCheckRobots(url);
  }
  const _response = await self._downloadUrl(url, false);
  ...
  if (statusCode >= 300 && statusCode < 400) {
    ...
    links = [urlMod.resolve(url, location)];
  } else {
    try {
      links = await self._fireHandlers(contentType, response.body, url);
    } catch (err) {
      self.emit("handlersError", err);
      throw new error.HandlersError("A handlers error occured. " + err.message);
    }
  }
